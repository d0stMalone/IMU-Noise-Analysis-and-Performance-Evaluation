...
def talker():
    ...
    while not rospy.is_shutdown():
        ...
        if "$VNYMR" in str(line):
            ...
            magx = float(new_data[4])
            magy = float(new_data[5])
            magz = float(new_data[6])
            ...
            imu_msg1.mag_field.magnetic_field.x = magx
            imu_msg1.mag_field.magnetic_field.y = magy
            imu_msg1.mag_field.magnetic_field.z = magz
